...
export default function NewProductPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      notify.success("Product created!", "New product has been added successfully")
      router.push("/admin/products")
    } catch (err: any) {
      setError(err.message)
      notify.error("Failed to create product", err.message || "Please try again")
    } finally {
      setLoading(false)
    }
  }
            ...
            <CardContent>
              {variants.length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-8">
                  No variants added. Click &quot;Add Variant&quot; to add product variants (size, color, etc.)
                </p>
              ) : (
